        ...
        for f in used_files:
            ...
            for msg in track:
                ...
                while acc >= resolution:
                    ...
                    if cur_vel == 0:
                        note_index = REST_INDEX
                    else:
                        
                        if MIN_PITCH <= cur_note <= MAX_PITCH:
                            note_index = cur_note - MIN_PITCH  # 0..62
                        else:
                            
                            note_index = REST_INDEX
                    if not (0 <= note_index <= REST_INDEX):
                        raise ValueError(
                            f"note_index {note_index} out of range [0, {REST_INDEX}]. "
                            f"cur_note={cur_note}, cur_vel={cur_vel}, "
                            f"MIN_PITCH={MIN_PITCH}, MAX_PITCH={MAX_PITCH}"
                        )
                    rows.append([note_index, cur_vel])
                    acc -= resolution
